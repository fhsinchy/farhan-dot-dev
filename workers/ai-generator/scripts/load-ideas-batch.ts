...
try {
	...
	const wranglerContent = readFileSync(wranglerTomlPath, 'utf-8');
	...
	if (!namespaceId || namespaceId.trim() === '') {
		throw new Error('IDEA_QUEUE namespace ID is empty in wrangler.toml. Please create the namespace and update the ID.');
	}
